...
let PersonasFormComponent = class PersonasFormComponent {
    constructor(fb, personasServices, router, router2 = _angular_router__WEBPACK_IMPORTED_MODULE_3__["ActivatedRoute"]) {
        this.fb = fb;
    ...
    ngOnInit() {
        ...
        this.router2.prototype.params.subscribe(params => {
            if (params["id"] == undefined) {
                return;
            }
            this.modoEdicion = true;
            this.personaID = params["id"];
            this.personasServices.getPersona(this.personaID.toString()).subscribe(persona => this.cargarFormulario(persona), error => console.error(error));
        });
    }
    save() {
        let persona = Object.assign({}, this.formGroup.value);
        console.table(persona);
        this.personasServices.createPersona(persona).subscribe(person => this.onSaveSF(), error => console.error(error));
    }
